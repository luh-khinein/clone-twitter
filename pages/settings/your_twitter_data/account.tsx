...
const Account: NextPage = () => {
  ...
  return (
    <SettingsLayout>
      <NavBar />
      <section className={`flex flex-col w-timeline h-screen items-start justify-start py-2 pb-10 border-r border-l ${backgroundTheme === 'light' ? 'border-gray-100' : backgroundTheme === 'black' ? 'border-zinc-800' : 'border-slate-800'}`}>
        <div className='flex items-center px-3 mb-5'>
          <button className={`p-2 flex items-center justify-center mr-5 duration-200 rounded-full ${backgroundTheme === 'light' ? 'hover:brightness-95' : backgroundTheme === 'black' ? 'hover:bg-zinc-800' : 'hover:brightness-110'}`} style={{

...
const TopicsFollowedButton: React.FC<TopicsFollowedValue> = ({ link, topic, description }) => {
  ...
  return (
    <div className='relative justify-end flex'>
      ...
      <button
        className={`translate-y-4 mr-3 absolute rounded-full py-2 w-24 font-bold border ${backgroundTheme === 'light' ? 'border-gray-100' : backgroundTheme === 'black' ? 'border-zinc-800' : 'border-slate-800'} before:content-['Following'] hover:before:content-['Unfollow'] hover:border-red-300 hover:text-red-500 hover:bg-red-100 duration-200`}
